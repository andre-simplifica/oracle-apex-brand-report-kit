...
Download `build-apex-brand-reports-0.1.2.zip`, verify the release and checksum, then extract its `build-apex-brand-reports` directory into `${CODEX_HOME:-$HOME/.codex}/skills/`. Restart or open a new Codex task so discovery refreshes. Install `requirements.txt` only if you choose to run the optional Python scripts.
